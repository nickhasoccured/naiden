...
module.exports = {
	...
	execute(client) {
		client.on("message", (message) => {
			...
			const guildCountChannel =
				db.get(`${message.guild.id}.countingChannel`) || false;
			if (!guildCountChannel) return;
			if (message.channel.id !== guildCountChannel) return;

			const currentNumber =
				db.get(`${message.guild.id}.countingNumber`) || 0;
			const expectedNumber = currentNumber + 1;

			const highScore =
				db.get(`${message.guild.id}.countingHighScore`) || 0;
			...
			if (message.content.trim().startsWith(expectedNumber)) {
				...
				message
					...
					.then((result) => {
						...
						if (expectedNumber > highScore) {
							db.set(
								`${message.guild.id}.countingHighScore`,
								expectedNumber
							);
						}
					});
				...
				const failEmbed = new Discord.MessageEmbed()
					.setTitle(
						`${message.member.displayName} failed at ${currentNumber}`
					)
					.setColor(config.theme.errorColor)
					.setDescription(
						`The expected number was ${expectedNumber}, now you have to start from 0!`
					);
				message.channel
					.send(`<@${message.author.id}>`, failEmbed)
					.catch((error) => {
		...
		client.on("messageDelete", (message) => {
			...
			message.channel.send(deletionEmbed).catch((error) => {
				console.error(`Failed to send message in #${channel.name} (${channel.id}) in ${channel.guild.name} (${channel.guild.id})
					* ${error}`);
			});
		});
