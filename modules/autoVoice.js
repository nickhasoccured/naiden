...
module.exports = {
	...
	execute(client) {
		client.on("voiceStateUpdate", async (oldState, newState) => {
			...
			if (
				!db.get(`${guild.id}.autoVoiceLobby`) ||
				!db.get(`${guild.id}.autoVoiceTemplate`) ||
				!db.get(`${guild.id}.autoVoiceCategory`)
			)
				return;

			let guildLobbyChannel =
				db.get(`${guild.id}.autoVoiceLobby`) || false;
			let guildTemplateChannel =
				db.get(`${guild.id}.autoVoiceTemplate`) || false;
			let guildVoiceCategory =
				db.get(`${guild.id}.autoVoiceCategory`) || false;
			...
			guildLobbyChannel = await client.channels
				.fetch(guildLobbyChannel)
				.then((result) => {
			...
			guildTemplateChannel = await client.channels
				.fetch(guildTemplateChannel)
				.then((result) => {
					if (result.type === "voice") return result;
				})
				.catch((error) => {
			...
			guildVoiceCategory = await client.channels
				.fetch(guildVoiceCategory)
				.then((result) => {
			...
			if (oldState.channel) {
				if (oldState.channel.parent) {
					if (oldState.channel.parent.id === guildVoiceCategory.id) {
						if (
							oldState.channel.id !== guildLobbyChannel.id &&
							oldState.channel.id !== guildTemplateChannel.id &&
				...
				if (newState.channel.id === guildLobbyChannel.id) {
					guildLobbyChannel
						...
						.then((result) => {
							...
							newState.setChannel(
								result,
								`${newState.member.displayName} created a new room`
							);
						})
						.catch((error) => {
